...
def calcTTTAdv_old (Config,station,Origin,flag,arrayname,Xcorrshift=None,Refshift=None):
    ...
    tt = getTravelTimes(delta=sdelta,depth=o_depth,model='ak135')

    for r in tt:
        if r['phase_name'] is Config[phasename] or r['phase_name'] == ('%sdiff')%(Config[phasename]):
            station.takeoff = r['take-off angle']
            ph = r['phase_name']
    
    #print 'STATIONINFO ',station,station.takeoff,ph   
    #logger.info('PROCESS %d STATION: %s --> DELTA: %f'% (flag,station,sdelta))
    ...
    output = open(tttname, 'w')
    pickle.dump(TTTGridMap, output)
    output.close()

    outputminmax = open('minmax-'+str(flag)+'.pkl','w')
    pickle.dump(k,outputminmax)
    outputminmax.close()
    
    stationpkl = open('station-'+str(flag)+'.pkl','w')
    pickle.dump(station,stationpkl)
    stationpkl.close()
    '''
# -------------------------------------------------------------------------------------------------    
